import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';

export const ForgotPasswordPage: React.FC = () => {
  const { forgotPassword, error, clearError } = useAuth();
  const [email, setEmail] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [success, setSuccess] = useState(false);
  const [validationError, setValidationError] = useState<string | null>(null);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    clearError();
    setValidationError(null);

    if (!email) {
      setValidationError('Email is required');
      return;
    }

    try {
      setIsSubmitting(true);
      await forgotPassword({ email });
      setSuccess(true);
    } catch (err) {
      // Handled by context error
    } finally {
      setIsSubmitting(false);
    }
  };

  if (success) {
    return (
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: '3rem', color: '#10b981', marginBottom: '1rem' }}>✓</div>
        <h3
          style={{ fontSize: '1.25rem', fontWeight: 600, color: '#f8fafc', marginBottom: '0.5rem' }}
        >
          Check your email
        </h3>
        <p style={{ color: '#94a3b8', fontSize: '0.875rem', lineHeight: '1.4rem' }}>
          If that email address exists in our system, we have sent a password reset token. In
          development, check the backend console log for the token string.
        </p>
        <div style={{ marginTop: '2rem' }}>
          <Link
            to="/login"
            style={{
              color: '#38bdf8',
              textDecoration: 'none',
              fontWeight: 500,
              fontSize: '0.875rem',
            }}
            onClick={clearError}
          >
            Back to Login
          </Link>
        </div>
      </div>
    );
  }

  return (
    <div>
      <h3
        style={{
          fontSize: '1.25rem',
          fontWeight: 600,
          color: '#f8fafc',
          margin: '0 0 1rem 0',
          textAlign: 'center',
        }}
      >
        Reset your password
      </h3>
      <p
        style={{
          color: '#94a3b8',
          fontSize: '0.875rem',
          lineHeight: '1.25rem',
          textAlign: 'center',
          marginBottom: '1.5rem',
        }}
      >
        Enter your email and we&apos;ll log a simulated password reset token in the terminal.
      </p>

      {/* Errors */}
      {(validationError || error) && (
        <div
          style={{
            backgroundColor: '#451a03',
            border: '1px solid #78350f',
            color: '#fef3c7',
            padding: '0.75rem 1rem',
            borderRadius: '8px',
            fontSize: '0.875rem',
            marginBottom: '1.5rem',
            lineHeight: '1.25rem',
          }}
        >
          {validationError || error}
        </div>
      )}

      <form
        onSubmit={handleSubmit}
        style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <label
            htmlFor="email"
            style={{ fontSize: '0.875rem', fontWeight: 500, color: '#cbd5e1' }}
          >
            Email address
          </label>
          <input
            id="email"
            type="email"
            value={email}
            onChange={(e) => {
              setEmail(e.target.value);
              setValidationError(null);
            }}
            style={{
              padding: '0.75rem 1rem',
              borderRadius: '8px',
              border: '1px solid #475569',
              backgroundColor: '#0f172a',
              color: '#f8fafc',
              fontSize: '0.875rem',
              outline: 'none',
            }}
            placeholder="name@example.com"
            required
          />
        </div>

        <button
          type="submit"
          disabled={isSubmitting}
          style={{
            padding: '0.75rem 1rem',
            borderRadius: '8px',
            backgroundColor: isSubmitting ? '#0284c7' : '#0ea5e9',
            color: '#ffffff',
            border: 'none',
            fontSize: '0.875rem',
            fontWeight: 600,
            cursor: isSubmitting ? 'not-allowed' : 'pointer',
            transition: 'background-color 0.2s',
            marginTop: '0.5rem',
          }}
        >
          {isSubmitting ? 'Requesting reset...' : 'Send Reset Token'}
        </button>
      </form>

      <div
        style={{ marginTop: '2rem', textAlign: 'center', fontSize: '0.875rem', color: '#94a3b8' }}
      >
        Remember your password?{' '}
        <Link
          to="/login"
          style={{ color: '#38bdf8', textDecoration: 'none', fontWeight: 500 }}
          onClick={clearError}
        >
          Log in
        </Link>
      </div>
    </div>
  );
};

export default ForgotPasswordPage;
